...
def convert(input_grid: list[str]) -> str:
    ...
    :param input_grid: - A matrix of strings representing OCR numbers.
    :return: - An equivalent string of digits to the OCR matrix given by `input_grid`.
    """
    OCR_PATTERNS = {
        (" _ ", "| |", "|_|", "   "): "0",
        ("   ", "  |", "  |", "   "): "1",
...
def split_OCR_digits(grid: list[str]) -> list[tuple[str]]:
    ...
    :param grid: - A matrix representing OCR letters.
    :return stacked: - A list of tuples containing only a single OCR "digit".
    """
    def _wrap(s: str, width: int):
        """Replace `wrap` from textwrap module since wrap wasn't working."""
        return [s[i:i+width] for i in range(0, len(s), width)]

    def split_OCR_digit_groups(grid: list[str]) -> list[tuple[str]]:
    ...
    split_grid = list(zip(*[_wrap(row, 3) for row in grid]))
...
def check_dimensions(grid: list[str]) -> None:
    """Raise a `ValueError` if the given `grid` has inappropriate dimensions.
